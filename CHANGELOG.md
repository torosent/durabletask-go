...
- Removed the `samples/sequence` and `samples/heterogeneous` samples. The activity sequence is demonstrated by `samples/durabletaskscheduler`, and the heterogeneous sample only demonstrated routing between embedded `backend.Executor` implementations, which has no Durable Task Scheduler equivalent.
...
- Removed the generated `tests/mocks` package. Nothing depended on it once the embedded host was gone; tests now use small hand-written fakes over the generated gRPC surfaces and over `backend.Executor`/`backend.EntityExecutor`.
- Removed `client.WithLegacyOrchestrationIDReusePolicyWire`. It existed only to send the legacy `IGNORE` reuse action to a known-compatible non-DTS server, and non-DTS servers are no longer supported. `IGNORE` now always fails closed with `client.ErrUnsupportedOrchestrationIDReusePolicy`; use `ERROR` or `TERMINATE`.
...
- `TaskHubGrpcClient` rejects the ambiguous legacy `IGNORE` reuse action. DTS reads the shared status field as a replacement policy, so `IGNORE` and `TERMINATE` are wire-identical; callers must use the representable `ERROR` or `TERMINATE` semantics.
- `backend` is now internal runtime plumbing rather than an extension point. It stays exported for source compatibility and retains only what the DTS client, DTS worker, and in-process task executor share: `Logger`/`DefaultLogger`/`NewSlogLogger`, `MetricsHooks` with `RetryMetric` and `HistoryMetric`, `Executor`/`ExecutionResults`, `EntityExecutor`, `WorkItemAbandonDelayError`, `EntityBatchFromRequestV2`, `HistoryEvent`, `OrchestrationRuntimeState` and its history helpers, `RebuildRewindHistory`, and the `ErrTaskHubExists`/`ErrTaskHubNotFound` sentinels the client maps gRPC statuses onto.
