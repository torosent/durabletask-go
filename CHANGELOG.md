...
- `backend` is now internal runtime plumbing rather than an extension point. It stays exported for source compatibility and retains only what the DTS client, DTS worker, and in-process task executor share: `Logger`/`DefaultLogger`/`NewSlogLogger`, `MetricsHooks` with `RetryMetric` and `HistoryMetric`, `Executor`/`ExecutionResults`, `EntityExecutor`, `WorkItemAbandonDelayError`, `EntityBatchFromRequestV2`, `HistoryEvent`, and the `ErrTaskHubExists`/`ErrTaskHubNotFound` sentinels the client maps gRPC statuses onto.
